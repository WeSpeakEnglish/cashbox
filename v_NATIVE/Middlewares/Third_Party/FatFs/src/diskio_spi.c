...
enum sd_speed { SD_SPEED_INVALID, SD_LO_SPEED, SD_HI_SPEED };
...
#define spi_cs_low() HAL_GPIO_WritePin(SD_CS_PORT, SD_CS_PIN, GPIO_PIN_RESET)
#define spi_cs_high() HAL_GPIO_WritePin(SD_CS_PORT, SD_CS_PIN, GPIO_PIN_SET)

static void spi_set_speed(enum sd_speed speed)
{
	int prescaler = SPI_BAUDRATEPRESCALER_128;
        
	if (speed == SD_LO_SPEED)
		prescaler = SPI_BAUDRATEPRESCALER_128;
	else if (speed == SD_HI_SPEED)
		prescaler = SPI_BAUDRATEPRESCALER_2;
        spi_change_speed(&HSPI, prescaler);
}
...
static u8 spi_txrx(u8 data)
{
	...
	//return SPI_SD->DR;
        ...
        u8 ret = 0xff;
        HAL_SPI_TransmitReceive (&HSPI, &data, &ret, 1, 0x1000);
        return ret;
}
...
static int sd_init(hwif *hw)
{
	...
	if (r == 0xff)
		{ print_f("fail spi\n"); return -1; }
	if (r == 0x01)
		print_f("success, SD v2.x\n");
	else if (r & 0x4) {
		hw->capabilities &= ~CAP_VER2_00;
		print_f("not implemented, SD v1.x\n");
	} else {
		print_f("fail\n");
		print_r1(r);
		return -2;
	...
	if (r != 0x01 && !(r & 0x4)) { /* allow it to not be implemented - old cards */
		...
		for (i=4; i<=23; i++)
			if (r3 & 1<<i)
				break;
		print_f("Vdd voltage window: %i.%i-", (12+i)/10, (12+i)%10);
		for (i=23; i>=4; i--)
			if (r3 & 1<<i)
				break;
		/* CCS shouldn't be valid here yet */
		print_f("%i.%iV, CCS:%li, power up status:%li\n",
				(13+i)/10, (13+i)%10,
				r3>>30 & 1, r3>>31);
		print_f("success\n");
	}
...
static int sd_get_data(hwif *hw, u8 *buf, int len)
{
	...
	if (_crc16 != calc_crc) {
		print_f("%s, crcs differ: %04x vs. %04x, len:%i\n", __func__, _crc16, calc_crc, len);
		return -1;
	}
...
static int sd_read_csd(hwif *hw)
{
	...
	spi_cs_high();
	if (r == -1) {
		print_f("failed to get csd\n");
		return -3;
	}

	if ((buf[0] >> 6) + 1 == 1) {
	/* CSD v1 */
	print_f("CSD: CSD v%i taac:%02x, nsac:%i, tran:%02x, classes:%02x%x, read_bl_len:%i, "
...
static int sd_read_cid(hwif *hw)
{
	...
	spi_cs_high();
	if (r == -1) {
		print_f("failed to get cid\n");
		return -3;
	}

	print_f("CID: mid:%x, oid:%c%c, pnm:%c%c%c%c%c, prv:%i.%i, psn:%02x%02x%02x%02x, mdt:%i/%i\n",
...
static int sd_readsector(hwif *hw, u32 address, u8 *buf)
{
	...
	return 0;
 fail:
	print_f("failed to read sector %li, err:%i\n", address, r);
	return r;
}
...
static int sd_writesector(hwif *hw, u32 address, const u8 *buf)
{
	...
	spi_cs_high();
	if (r != 0) {
		print_f("sd_put_data returned: %i\n", r);
		r = -3;
		goto fail;
	}
	...
	return 0;
 fail:
	print_f("failed to write sector %li, err:%i\n", address, r);
	return r;
}
...
int sd_read(hwif* hw, u32 address, u8 *buf)
{
	...
	if (tries == -1)
		print_f("%s: couldn't read sector %li\n", __func__, address);
...
int sd_write(hwif* hw, u32 address,const u8 *buf)
{
	...
	if (tries == -1)
		print_f("%s: couldn't write sector %li\n", __func__, address);
