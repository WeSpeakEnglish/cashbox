...
unsigned char * frame = (unsigned char*)Modbus.buffer; 
...
{
  uint8_t i;
	if (Modbus.readyBuff) // is there something to check?
	{       
          for(i = 0; i < Modbus.readySymbols; i++){
...
void sendPacket(unsigned char bufferSize)
{
	HAL_UART_Transmit_DMA(&huart3, (uint8_t *) frame, bufferSize);
//	usDelay(frameDelay);
